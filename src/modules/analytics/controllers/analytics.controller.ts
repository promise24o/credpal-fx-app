...
export class AnalyticsController {
  ...
  async getDashboardOverview() {
    return this.analyticsOperationsService.getDashboardOverview();
  }
  ...
  async getTransactionStats(@Query('period') period: 'day' | 'week' | 'month' | 'year' = 'month') {
    return this.analyticsOperationsService.getTransactionStats(period);
  }
  ...
  async getCurrencyStats() {
    return this.analyticsOperationsService.getCurrencyStats();
  }
  ...
  async getTopTraders(@Query('limit') limit: string = '10') {
    return this.analyticsOperationsService.getTopTraders();
  }
  ...
  async getDailyVolume(@Query('days') days: string = '30') {
    return this.analyticsOperationsService.getDailyVolume();
  }
  ...
  async getFailedTransactions() {
    return this.analyticsOperationsService.getFailedTransactions();
  }
}
